...
Running `sync_agents.sh` then produces `.claude/`, `.codex/`, `.opencode/`, `.agents/skills/`, and a root `AGENTS.md`.
...
- **Surfaces** — drop any of Claude/Codex/OpenCode at setup time if you only use one or two.
...
- The **Codex** and **OpenCode** emitters target those tools' config formats as of early 2026. The generated output was structurally verified, but has not been runtime-tested inside Codex or OpenCode in this build. Treat those surfaces as best-effort.
