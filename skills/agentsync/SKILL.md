...
**This skill is agentsync `0.2.4`.** (Release chore: bump this string with every version.) Bootstrap stamps it into `agents/agentsync.conf` as `AGENTSYNC_VERSION`. On reconcile, compare the project's stamped version against this one — if this is newer, apply the intervening versions' enhancements (see [Upgrades by version](#upgrades-by-version)) and re-stamp.
...
### Skills
- grill-plan
- orchestrate (Claude only — drives the engineer/code-reviewer loop)
...
- **0.2.4** — New **orchestrate** skill (`templates/skills/orchestrate/`): a Claude-only delegator that drives an approved plan to clean, verified implementation by looping `engineer` → `code-reviewer` → `engineer`, phase by phase, from the main session. Copy it into `agents/skills/orchestrate/` if absent. It's marked Claude-only via `context:` frontmatter, so `is_delegator_skill` keeps it out of the Codex/OpenCode/GitHub fan-out automatically — no per-surface variant. After adding it, re-sync so it lands in `.claude/skills/`.
